...
_lr_productions = [
  ...
  ('stmts -> stmts stmt','stmts',2,'p_stmts','myparser.py',40),
  ('stmts -> empty','stmts',1,'p_stmts','myparser.py',41),
  ('stmt -> assignment','stmt',1,'p_statement','myparser.py',51),
  ('stmt -> for','stmt',1,'p_statement','myparser.py',52),
  ('stmt -> if','stmt',1,'p_statement','myparser.py',53),
  ('stmt -> while','stmt',1,'p_statement','myparser.py',54),
  ('stmt -> expression','stmt',1,'p_statement','myparser.py',55),
  ('if -> IF expression COLON LBRACE stmts RBRACE elif','if',7,'p_if','myparser.py',61),
  ('elif -> ELIF expression COLON LBRACE stmts RBRACE elif','elif',7,'p_elif','myparser.py',67),
  ('elif -> else','elif',1,'p_elif','myparser.py',68),
  ('else -> ELSE COLON LBRACE stmts RBRACE','else',5,'p_else','myparser.py',77),
  ('else -> empty','else',1,'p_else','myparser.py',78),
  ('while -> WHILE expression COLON LBRACE stmts RBRACE','while',6,'p_while','myparser.py',85),
  ('for -> FOR ID IN RANGE LPAREN NUMBER RPAREN COLON LBRACE stmts RBRACE','for',11,'p_for','myparser.py',91),
  ('assignment -> ID ASSIGN expression','assignment',3,'p_assignment','myparser.py',97),
  ('expression -> expression PLUS term','expression',3,'p_expr_operator','myparser.py',103),
  ('expression -> expression MINUS term','expression',3,'p_expr_operator','myparser.py',104),
  ('expression -> expression GE term','expression',3,'p_expr_operator','myparser.py',105),
  ('expression -> expression EQ term','expression',3,'p_expr_operator','myparser.py',106),
  ('expression -> expression NE term','expression',3,'p_expr_operator','myparser.py',107),
  ('expression -> expression LT term','expression',3,'p_expr_operator','myparser.py',108),
  ('expression -> expression LE term','expression',3,'p_expr_operator','myparser.py',109),
  ('expression -> expression GT term','expression',3,'p_expr_operator','myparser.py',110),
  ('expression -> term','expression',1,'p_expr_operator','myparser.py',111),
  ('term -> term TIMES factor','term',3,'p_term','myparser.py',120),
  ('term -> term DIVIDE factor','term',3,'p_term','myparser.py',121),
  ('term -> factor','term',1,'p_term','myparser.py',122),
  ('factor -> NUMBER','factor',1,'p_factor','myparser.py',131),
  ('factor -> ID','factor',1,'p_factor','myparser.py',132),
  ('factor -> LPAREN expression RPAREN','factor',3,'p_factor','myparser.py',133),
  ('empty -> <empty>','empty',0,'p_empty','myparser.py',141),
]
